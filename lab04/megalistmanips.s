...
map:
    addi sp, sp, -24 # Need 12 more bytes to save temporary registers (BUG 3)
    sw ra, 0(sp)
    sw s1, 4(sp)
    sw s0, 8(sp)
...
mapLoop:
    lw t1, 0(s0)        # load the address of the array of current node into t1 # BUG 1! (BEFORE was add t1, s0, x0)
    lw t2, 4(s0)        # load the size of the node's array into t2
    
    slli t3, t0, 2      # offset the array address by the count # BUG 2! (BEFORE was add t1, t1, t0)
    add t1, t1, t3      # Actually need to do t1 = t1 + (4 * t0)... So t3 = 4 * t0
    ...
    sw a0, 0(t1)        # store the returned value back into the array # BUG 3! (The temporary variables might have changed after func call... Need to save)
    addi t0, t0, 1      # increment the count
    bne t0, t2, mapLoop # repeat if we haven't reached the array size yet

    lw a0, 8(s0)        # load the address of the next node into a0 ## BUG 4! (BEFORE was la a0, 8(s0))
    mv a1, s1           # put the address of the function back into a1 to prepare for the recursion ## BUG 5! (BEFORE was lw a1, 0(s1))
    ...
    lw s1, 4(sp)
    lw ra, 0(sp)
    addi sp, sp, 24     # SOLVE BUG 3 (it was 12)
    
    jr ra               # BUG 6! Missing return (BEFORE was nothing)
